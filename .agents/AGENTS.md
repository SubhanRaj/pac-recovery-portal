...
3.  **Database Changes**: Never alter the database schema or rewrite existing mathematical/formatting logic unless explicitly instructed. `pac_dues` is an append-only ledger (see [PLAN.md](../PLAN.md)) — no `period`, no lock/unlock; a district can have arbitrarily many entries, and none are ever edited or deleted in place.
4.  **Remote/deploy actions require a fresh explicit go-ahead every time**: never run a `--remote` D1 command or `wrangler deploy` without the user saying so for that specific change. There is no CI/CD — a `git push` to `main` does not deploy anything by itself. Local `--local` D1 work and `pnpm run preview` don't need to ask.
...
6.  **Never commit anything under `scripts_and_data/`** that isn't excluded by an existing `.gitignore` pattern (`*.sql`, `*.csv`, `*.txt`, `*.py`, `*.xlsx`, `*.xls`, `*hash*`, `backups/`) — that directory holds the department's real officer contact data, D1 export backups, and source Excel workbooks. See CLAUDE.md's Security section for why this is a hard rule, not a style preference.
